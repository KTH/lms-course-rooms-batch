...
module.exports = {
  ...
  async sendEnrollments(enrollments) {
    ...
    if (enrollments.length > normalized.length) {
      log.error(
        `SIS IMPORT: There are ${
          enrollments.length - normalized.length
        } enrollments that are not going to be sent to Canvas due to incorrect formatting. Please check what you send to "Canvas.sendEnrollments"`
      );
    }
